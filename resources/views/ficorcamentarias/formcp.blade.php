<div class="card p-3">  
    ...
    <hr>
    <div class="form-row">
        <div class="form-group col-md-12"><strong>As informações acima se referem ao que foi preenchido no formulário anterior. Abaixo, preencha com as informações da Contra-Partida do Lançamento da Ficha Orçamentária.</strong></div>
    </div>
    <br>
...
<div class="form-row">
    <div class="form-group col-md-9">
        <label for="conta">Escolha uma Conta: {{ $tipocontaid_quantidades_key }} </label>
        <input list="contas_{{ $tipocontaid_quantidades_key }}_{{ $i }}" name="conta_id_{{ $tipocontaid_quantidades_key }}_{{ $i }}" id="conta_id_{{ $tipocontaid_quantidades_key }}_{{ $i }}" class="form-control" value="{{ $lancamento->conta_id ?? old('conta_id') }}">
                ...
                @foreach($tipocontaid_descricaoconta as $tipocontaid_descricaoconta_key=>$tipocontaid_descricaoconta_value)
                    @foreach($lista_contas as $lista_conta)
                        @if(($tipocontaid_quantidades_key == $lista_conta->tipoconta_id) and ($tipocontaid_descricaoconta_key == $lista_conta->tipoconta_id))
                            <option value="{{ $lista_conta->id }}">{{ $lista_conta->nome }}---{{$tipocontaid_descricaoconta_key}}--{{ $lista_conta->tipoconta_id }}</option>
                        @endif
                    @endforeach
                @endforeach
        </datalist>
